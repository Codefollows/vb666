...
class vB5_Frontend_Controller_Page extends vB5_Frontend_Controller
{
	public function index($pageid)
	{
		...
		// The register page may need to set a cookie; do this before echoing anything
		if (!empty($_REQUEST['routestring']) AND $_REQUEST['routestring'] === 'register' AND !empty($_REQUEST['ref']))
		{
			$referralCode = $api->callApi('referuser', 'validateReferralCode', [$_REQUEST['ref']]);
			if ($referralCode)
			{
				// The cookie is set for 30 days. This is independent of the setting
				// for how long a referral code is valid. The user must register within
				// 30 days of when they followed the referral link for it to work. In
				// any case, the validity of the actual referral code is checked during
				// registration, which is what is used to assign referrals.
				vB5_Cookie::set('referralcode', $referralCode, 30);
			}
		}
		...
		$api->invokeHook('hookGetPageMain', [
			'pageid' => $pageid,
		...
		if (!empty($_REQUEST['goto']) AND $_REQUEST['goto'] == 'newpost' AND !empty($arguments['nodeid']) AND !empty($arguments['channelid']))
		{
			if ($this->vboptions['threadmarking'] AND vB5_User::get('userid'))
			{
				...
				$channelRead = $api->callApi('node', 'getNodeReadTime', [$arguments['channelid']]);
				$topicRead = $api->callApi('node', 'getNodeReadTime', [$arguments['nodeid']]);
			...
			$goToNodeId = $api->callApi('node', 'getFirstChildAfterTime', [$arguments['nodeid'], $topicView]);

			if (empty($goToNodeId))
			{
				$thread = $api->callApi('node', 'getNodes', [[$arguments['nodeid']]]);
			...
			if ($goToNodeId)
			{
				...
				$urlKey = $urlCache->register($router->getRouteId(), ['nodeid' => $arguments['nodeid']], ['p' => $goToNodeId]);
				$replacements = $urlCache->finalBuildUrls([$urlKey]);
		...
		*/
		...
		$api->callApi('page', 'savePreCacheInfo', [$pageKey]);
	 ...
	 */
	public static function renderTemplate($templateName, $data = [], $isParentTemplate=true)
	{
