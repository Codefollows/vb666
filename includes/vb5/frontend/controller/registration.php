...
class vB5_Frontend_Controller_Registration extends vB5_Frontend_Controller
{
	...
	public function actionRegistration()
	{
		...
		// add referral code
		$referralCode = vB5_Cookie::get('referralcode', vB5_Cookie::TYPE_STRING);
		if (!empty($referralCode))
		{
			$data['user']['referralcode'] = $referralCode;
		}
		...
		if ($abort)
		{
			if(isset($abort['errors']))
			{
				$this->sendAsJson(['errors' => $abort['errors']]);
			}
			return;
		}
	...
	private function loginUser($api, $vboptions, $userid, $password)
	{
		...
		vB5_Cookie::setSessionCookies($loginInfo, '', true);
		return true;
	}
	...
	public function actionActivateUser()
	{
		...
		$api = Api_InterfaceAbstract::instance();
		$result = $api->callApi('user', 'activateUser', ['userid' => $get['u'], 'activateid' => $get['i']]);
		...
		{
			$phraseArgs = is_array($result['errors'][0]) ? $result['errors'][0] : [$result['errors'][0]];
		}
		else
		{
			$phraseArgs = is_array($result) ? $result : [$result];
		}
		$messagevar = call_user_func_array([$phraseController, 'getPhrase'], array_values($phraseArgs));
	...
	public function actionActivateForm()
	{
		...
		$post = [
			...
			'activateid' => trim($_POST['activateid'] ??  ''), // Activate ID
		];

		$api = Api_InterfaceAbstract::instance();
		$result = $api->callApi('user', 'activateUserByUsername', $post);
	...
	public function actionActivateEmail()
	{
		...
		$api = Api_InterfaceAbstract::instance();
		$result = $api->callApi('user', 'sendActivateEmail', $input);
		...
		$this->sendAsJson(['msg' => 'lostactivatecode']);
	}
	...
	public function actionDeleteActivation()
	{
		...
		$messagevar = call_user_func_array(array($phraseController, 'getPhrase'), array_values($phraseArgs));
		vB5_ApplicationAbstract::showMsgPage($phraseController->getPhrase('registration'), $messagevar);
	}
