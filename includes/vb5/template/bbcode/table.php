...
class vB5_Template_BbCode_Table
{
	...
	*/
	protected $table_param_list = [
		...
		'class' => [
			'callback' => ['$this', 'validateTableClass']
		],
		'align' => [
	...
	*/
	public function parseTableTag($content, $params = '')
	{
		...
		foreach (['border-width', 'padding'] AS $item)
		{
		...
		// add .b-bbcode__table to table. Note that this effects both regular render & wysiwyg render currently.
		// Wysiwyg also further adds its own "wysiwyg_dashes" class to the table & all children elements, see
		// vB5_Template_BbCode_Tablewysiwyg::modifyTableParams()
		$class = $table_params['attributes']['class'];
		$table_params['attributes']['class'] = 'b-bbcode__table' . ($class ? ' ' : '') . $class;
		...
		// Note that the wysiwig version has $this->wrap_table = false so it won't get the wrapper...
		if ($this->wrap_table)
		{
			$table = "<div class=\"b-bbcode__table--wrapper " . $this->table_class_prefix . "\">$table</div>";
		}
	...
	*
	* @return	array	['css' => [<param> => <value>, ...], 'attributes' => [<param> => <value>, ...] ]
	*/
	...
	*/
	public function isValidNamedParam(&$value, $config, $context = '')
	{
		...
		if (!empty($config['callback']))
		{
			...
			if (!call_user_func_array($config['callback'], [&$value, $context]))
			{
				return false;
			}
		}
