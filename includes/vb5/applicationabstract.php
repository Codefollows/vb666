...
EOD;
	...
	//the child classes will define this as public but calling it directly on this class
	//won't so anybody any good.
	protected static function init($rootDir)
	{
		//we might need to adjust this if we ever have parameters to the constructor
		//for different applications but given that we go through the init function it
		//seems unlikely.
		if (!self::$instance)
		{
			...
			require_once($rootDir . '/core/vb/vb.php');
			vB::init();
	...
	protected static function setHeaders()
	{
		...
		try
		{
			...
			$options = $api->callApi('options', 'fetchValues', [[
				'nocacheheaders',
				'clickjackingheaders',
	...
	protected function convertInputArrayCharset()
	{
		...
		if ($requestcharset AND !$stringUtil->isDefaultCharset($requestcharset))
		{
			$routestring = $_REQUEST['routestring'] ?? null;

			$boardCharset = $stringUtil->getCharset();
	...
	public static function handleException($exception, $simple = false)
	{
		...
		try
		{
			...
			if ($config->debug)
			{
				...
				if (!$simple)
				{
					$phrase = $api->callApi('phrase', 'fetch', ['phrase' => $message]);
			...
			if ($exception instanceof vB5_Exception_404)
			{
				...
				$api->invokeHook('hookFrontendOn404', ['path' => $path, 'normalErrorResponse' => &$normalErrorResponse]);
			}
	...
	{
		$api = Api_InterfaceAbstract::instance();
		$canusesitebuilder = $api->callApi('user', 'hasPermissions', ['adminpermissions', 'canusesitebuilder']);
	...
	protected static function showErrorForm($error, $url)
	{
		...
		if (is_array($error))
		{
			$args = is_array($error[1]) ? $error[1] : [$error[1]];
			$error = $error[0];
		}
		$templater->register('error', $error);
	...
	public static function checkState($route = [])
	{
		$response = Api_InterfaceAbstract::instance()->callApi('state', 'checkBeforeView', ['route' => $route]);
		if ($response)
		{
			if (self::isAjax())
			{
				...
				else
				{
					...
					$output = self::prepJsonSend(['error' => ['error_x', $response['msg']], 'state' => $response['state']]);
				}
	 ...
	 */
	public static function showMsgPage($title = '', $message = '', $state = '')
	{
		self::showMessagePageInternal('message_page', $title, $message, $state);
	}
	 ...
	 * @param string $state The state of the site
	 */
	public static function showMsgPageBare($title = '', $message = '', $state = '')
	{
		self::showMessagePageInternal('message_page_bare', $title, $message, $state);
	}

	private static function showMessagePageInternal($template, $title, $message, $state)
	{
		$page = [];
		$page['routeInfo'] = self::getRouteInfo();
		$page['ignore_np_notices'] = self::getIgnoreNPNotices();
	...
	public static function getPreheader()
	{
		...
		$templater = new vB5_Template('page_preheader');
		...
		Api_InterfaceAbstract::instance()->invokeHook('hookFrontendPreheader', ['preheaderHtml' => &$html]);
	 ...
	 */
	public function sendAsJsonAndCloseConnection($responseToSendAsJson = [])
	{
