...
if ($_REQUEST['do'] == 'modify')
{
	...
	// sort by default liftdate, limit what it can be.

	switch ($vbulletin->GPC['sortby'])
	{
		case 'bandate':
		case 'liftdate':
			// query sanitization is done via this implicit whitelist, but leaving the currently pointless ` filter
			// as a reminder in case this changes later.
			$sortby_safe = '`userban`.`' . str_replace('`', '', $vbulletin->GPC['sortby']) . '`';
			break;
		default:
			$sortby_safe = '`userban`.`liftdate`';
			break;
	}

	// Default ASC, only can be ASC or DESC
	$sortdir_safe = 'ASC';
	if (strtolower($vbulletin->GPC['sortdir']) == 'desc')
	{
		$sortdir_safe = 'DESC';
	}
	...
	// define the column headings. Keys used by add_sorting_to_header() below.
	$headercell = [
		               $vbphrase['username'],
		               $vbphrase['banned_by'],
		'bandate'   => $vbphrase['banned_on'],
		               $vbphrase['ban_period'],
		'liftdate'  => $vbphrase['ban_will_be_lifted_on'],
	...
	function get_banning_link($content, $sortby, $sortdir)
	{
		$url = "modcp/banning.php?do=modify&sortby=$sortby&sortdir=$sortdir";
		$url = htmlspecialchars($url);
		// position relative required to align the sort direction icon properly.
		return "<a href=\"$url\" class=\"h-relative\" >$content</a>";
	}

	function add_sorting_to_header(&$headercells, $currentsortby, $currentsortdir)
	{
		foreach ($headercells AS $__k => $__phrase)
		{
			if ($__k === $currentsortby)
			{
				$flipped = (strtolower($currentsortdir) == 'desc' ? 'ASC' : 'DESC');
				$sortmarker = get_sortdir_icon($currentsortdir);
				// append sort marker & flip direction link
				$headercells[$__k] = get_banning_link($__phrase . $sortmarker, $currentsortby, $flipped);
			}
			else if (is_string($__k))
			{
				// append sort links
				$headercells[$__k] = get_banning_link($__phrase, $__k, 'ASC');
			}
		}
	}
	add_sorting_to_header($headercell, $vbulletin->GPC['sortby'], $vbulletin->GPC['sortdir']);
	...
	$tempusers = $vbulletin->db->query_read("
		...
		ORDER BY $sortby_safe $sortdir_safe, user.username
	");
