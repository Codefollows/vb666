...
class vB5_Route_Channel extends vB5_Route
{
	...
	public static function validatePrefix($data)
	{
		...
		if (!isset($data['prefix']))
		{
			...
			{
				$url = self::createPrefix($data['parentid'], $data['title']);
			}
			else
			{
				$url = self::createPrefix($data['parentid'], $data['urlident']);
			}
	 ...
	 */
	public static function createPrefix($parentId, $title)
	{
		...
		$url .= vB_String::getChannelUrlIdent($title);
	...
	protected static function validInput(array &$data)
	{
		...
		if (!isset($data['prefix']))
		{
			...
			if (empty($data['urlident']))
			{
				$url = self::createPrefix($newChannel['parentid'], $newChannel['title']);
			}
			else
			{
				$url = self::createPrefix($newChannel['parentid'], $newChannel['urlident']);
			}
			$data['prefix'] = $url;
		}
	...
	protected static function updateContentRoute($oldRouteInfo, $newRouteInfo)
	{
		...
		$updateIds = $db->assertQuery('vBForum:node', [
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			vB_dB_Query::COLUMNS_KEY => ['nodeid'],
			vB_dB_Query::CONDITIONS_KEY => ['routeid' => $oldRouteInfo['routeid']],
		]);
		...
		if (!empty($updateIds))
		{
			...
			$db->update('vBForum:node', ['routeid' => $newRouteInfo['routeid']], ['nodeid' => $nodeIds]);
		}
		...
		$routes = $db->getRows('routenew', [
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			vB_dB_Query::CONDITIONS_KEY => [
				['field'=>'class',	'value' => ['vB5_Route_Conversation', 'vB5_Route_Article']],
				['field'=>'prefix','value' => $oldRouteInfo['prefix']],
				['field'=>'contentid',	'value' => $oldRouteInfo['channelid']],
		...
		foreach ($routes AS $route)
		{
			...
			if ($route['name'])
			{
				$db->update('routenew', ['name' => vB_dB_Query::VALUE_ISNULL], ['routeid' => $route['routeid']]);
			}
			...
			$args = vb_unserialize($route['arguments']);
			if (isset($args['pageid']))
			{
				$pageid = $args['pageid'];
				$events[] = "pageChg_$pageid";
				$db->update('page', ['routeid' => $newConversationRouteid], ['pageid' => $pageid]);
			}
			...
			$updateIds = $db->assertQuery('vBForum:node', [
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
				vB_dB_Query::COLUMNS_KEY => ['nodeid'],
				vB_dB_Query::CONDITIONS_KEY => ['routeid' => $route['routeid']],
			]);
			...
			if (!empty($updateIds))
			{
				...
				$db->update('vBForum:node', ['routeid' => $newConversationRouteid], ['nodeid' => $nodeIds]);
			}
	...
	protected static function updateSecondaryHomepageRoutes($oldRouteInfo, $newRouteInfo)
	{
		...
		if ($newRouteInfo['ishomeroute'])
		{
			...
			$conditions = [
				['field'=>'class',	'value' => ['vB5_Route_Conversation', 'vB5_Route_Article']],
				['field'=>'prefix','value' => $newRouteInfo['prefix']],
				['field'=>'contentid',	'value' => $oldRouteInfo['channelid']],
				['field'=>'redirect301', 'operator' => vB_dB_Query::OPERATOR_ISNULL],
			];

			$route = $db->getRow('routenew', [
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			...
			if ($route)
			{
				$db->update('routenew', ['ishomeroute' => 1], ['routeid' => $route['routeid']]);
