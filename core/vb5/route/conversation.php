...
class vB5_Route_Conversation extends vB5_Route
{
	...
	public function __construct($routeInfo, $matches, $queryString = '', $anchor = '')
	{
		...
		if (isset($this->arguments['channelid']))
		{
			...
			if (empty($channelInfo))
			{
				...
				$channelInfo = [
					...
					'styleid' => $channel['styleid'],
					'options' => $channel['options'],
					'routeid' => $channel['routeid'],

					'rss_enabled' => $channel['rss_enabled'],
					'rss_route' => $channel['rss_route'],
					'title' => $channel['title'],
				];
				$cache->write($hashKey, $channelInfo, 1440, ['routeChg_' . $channelInfo['routeid'], 'nodeChg_' . $channelInfo['nodeid']]);
			}
		...
		$this->arguments['pageSchema'] = 'ItemPage';
	}
	...
	protected function checkRoutePermissions()
	{
		...
		if (!$contentLib->validate($node, vB_Library_Content::ACTION_VIEW, $node['nodeid'], [$node['nodeid'] => $node]) AND empty($node['public_preview']))
		{
			throw new vB_Exception_NodePermission($node['nodeid']);
		}
	}

	private static function getChannelURL($channelid, $node = [])
	{
		$cache = vB_Cache::instance(vB_Cache::CACHE_FAST);
		...
		elseif (!empty($node['channelroute']))
		{
			...
			$cache->write($hashKey, $channelInfo, 1440, ['routeChg_' . $node['channelroute'], 'nodeChg_' . $node['channelroute']]);
		}
		 */
		else
		{
	...
	protected static function updateContentRoute($oldRouteInfo, $newRouteInfo)
	{
		...
		{
			$arguments= vb_unserialize($arguments);
		}
		...
		{
			$oldRouteInfo['arguments'] = vb_unserialize($oldRouteInfo['arguments']);
		}
		...
		else
		{
			...
			{
				$db->update('routenew', ['ishomeroute' => 0], ['routeid' => $newRouteInfo['routeid']]);
			}
		}
		...
		if (!empty($arguments['customUrl']))
		{
			...
			catch(vB_Exception_Api $e)
			{
				...
				if ($e->has_error('invalid_node_id'))
				{
					...
					$page = $db->getRow('page', ['pageid' => $arguments['pageid']]);
	...
	private static function updateNodeRoutes($db, $topicid, $oldrouteid, $newrouteid)
	{
		...
		$nodeids = $db->getColumn('vBForum:getChildrenMatchingRoute', 'nodeid', [
			'nodeid' => $topicid,
			'routeid' => $oldrouteid,
		]);
		...
		$db->update('vBForum:node', ['routeid' => $newrouteid], ['nodeid' => $nodeids]);
		return $events;
	}
	...
	public function getUrl()
	{
		...
		else
		{
			...
			if (empty($urlident) OR !is_array($urlident))
			{
				...
				$cache->write($hashKey, $urlident, 1440, ['routeChg_' . $this->routeId, 'nodeChg_' . $this->arguments['nodeid']]);
			}
