...
class vBSphinxSearch_Core extends vB_Search_Core
{
	...
	// some extensions depend on these being protected even though the core code does not.
	protected $sphinxDB;
	protected $table;
	protected $what = ['id'];
	protected $match = '';
	...
	protected $groupby = '';
	protected $sort = [];
	protected $done_permission_check;

	// no extension uses these properties yet.
	private $filters = [];
	...
	protected $attributes_definition = [
		'contenttypeid'        => 'rt_attr_uint',
		'lastcontentid'        => 'rt_attr_uint',
		'lastauthorid'         => 'rt_attr_uint',
		...
		'starterlastcontent'   => 'rt_attr_uint', // ...
		'starterpublishdate'   => 'rt_attr_uint',
		'startercreated'       => 'rt_attr_uint',
		...
		'votes'                => 'rt_attr_uint',
		'lastcontent'          => 'rt_attr_uint',
		'created'              => 'rt_attr_uint',
		'publishdate'          => 'rt_attr_uint',
		'unpublishdate'        => 'rt_attr_uint',
		...
		'eventstartdate'       => 'rt_attr_uint',
		'eventenddate'         => 'rt_attr_uint',
		'titlesort'            => 'rt_attr_string',
		'hasanswer'            => 'rt_attr_uint',
	];
	...
	/**************SEARCHING**************************/
	protected function useSphinxSearch($criteria) : bool
	{
	...
	public function getResults(vB_Search_Criteria $criteria)
	{
		...
		// Fall back to DB search unless conditions (default: has keywords) are met.
		if (!$this->useSphinxSearch($criteria))
		{
			$vBDBSearch_Core = new vBDBSearch_Core();
			return $vBDBSearch_Core->getResults($criteria);
		}

		$rawKeywords = $criteria->get_raw_keywords();
		...
		$keywords = $this->adjustKeywordsForSphinx($rawKeywords);
		...
		// Default sphinxsearchcore will fall back to DB search if there are no keywords. However,
		// we want to allow extensions to let sphinxsearch handle certain complex sorting even if there are
		// no keyword searches.
		if (!empty($keywords))
		{
			// Using a separate var rather than doing something like $this->where['MATCH'], because setting &
			// unsetting it in the where array before imploding would require array copying etc & is messy.
			$this->match = "MATCH('$field" . $this->sphinxEscapeKeywords($keywords) . "')";
		}
		else
		{
			$this->match = '';
		...
		if (!empty($keywords))
		{
			$this->sphinxDB->suppressSyntaxErrorsNextQuery();
		}

		$commentTag = "\n\t\t\t/**" . __FUNCTION__ . (defined('THIS_SCRIPT') ? '- ' . THIS_SCRIPT : '') . "**/";
		$query = $this->generateSearchQuery($query_limit) . $commentTag;
		$res = $this->sphinxDB->query($query);
		...
		if (!empty($keywords) AND $this->sphinxDB->lastQueryHadSyntaxError())
		{
			/*
			Notes on "... syntax error, unexpected '-'" or "... unexpected $end near ''..."

			These errors seems to most frequently occur for cases like "something -- something else".
			Certain characters are sphinx match operators (e.g. | for OR, - for word negation)
			so we should not escape them if we want users to be able to utilize the advanced
			search operations. However, if it's like a dash as part of a compound word or
			double dashes syntatically equivalent to an emdash, that just breaks the
			search because sphinx can't understand what was meant.
			In that case, let's assume that the user meant those characters literally and
			not as operators, and do a full escape on the keywords string.
			Note that on the first pass, words like "OR" and "NOT" are converted to the sphinx
			operators "|" and "-" (see adjustKeywordsForSphinx()). For the second pass, perhaps
			we should skip that and again assume that the user meant those words literally.

			As such, skip adjustKeywordsForSphinx() (this is not a security function, but just a
			utility/usability), and use sphinxEscapeString() instead of sphinxEscapeKeywords()
			*/
			$this->match = "MATCH('$field" . $this->sphinxEscapeString($rawKeywords) . "')";
	...
	private function generateSearchQuery($query_limit)
	{
		$groupby = '';
		...
		$query = '
			...
			FROM ' . $this->table . '
			WHERE ' . $this->generateWhereClause() . $groupby . '
	...
	{
		/*
		Previous code wants MATCH to always be the first item in the where clause.
		I thought I recalled this was due to sphinx erroring out when certain where conditions
		came before the MATCH(), but I can't seem to reproduce this on testing. It may
		depend on the particular columns or conditions.
		Without knowing more, I do not want to change this behavior.
		I want a better way than "assume it's at 0th position" to guarantee that we're editing
		the correct element when we swap the match keywords out and keep everything else in
		place.
		 */
		if (!empty($this->match))
		{
		...
		foreach ($this->filters[$filter_method] as $field => $value)
		{
			switch ($field)
			{
				...
				case 'follow':
					...
					break;
				case 'unpublishdate':
					// todo: this node. prefix is probably a bug and will likely cause a "unknown column: node" error
...
// 					$this->where[''] = "(" . implode(' OR ', $qbits) . ")";
	...
	protected function process_sort(vB_Search_Criteria &$criteria)
	{
		...
		foreach ($sort_array AS $sort => $direction)
		{
			...
			if ($sort == "isstarter")
			{
				if (!$is_starter_only)
				{
					$this->sort['is_starter2'] =  'is_starter2 ' . $direction;
			...
			if (isset($sort_map[$sort]))
			{
				...
				if ($sort_field == 'rank')
				{
					// Multiple sort fields can map to 'rank'. Keyed to dedupe to avoid
					// "alias 'rank' must be unique" sphinxql error.
					$this->what['sort_rank'] = "weight() as rank";
				}
				else
				{
					if ($viewTopics)
				...
				$this->sort[$sort_field] = $sort_field . ' ' . $direction;
			}
		}
	}
	...
	private function doShowApprovedCheck()
	{
		// no need to do this multiple times.
		if (!empty($this->where['showapprovedcheck']))
		{
			return;
		}
		...
		if (empty($userid))
		{
			$this->where['showapprovedcheck'] = '(showapproved > 0)';
		}
		else
		{
			// Allow user to see their own, unapproved nodes
			// OR operator in WHERE clause is unsupported in sphinx, so we have to get around it by
			// adding a selected field then sort on that. Not great for indexed based sorting but
			// shouldn't be a problem for sphinx.
			$this->what['showapprovedcheck'] = "(showapproved > 0 OR userid = $userid) AS sa";
			$this->where['showapprovedcheck'] = 'sa = 1';
		}
	}
	...
	protected function process_channel_filters($channelid, $depth, $include_starter, $depth_exact, $excludeUserSpecific)
	{
		...
		)
		{
			...
			if (!empty($userId))
			{
				...
				if (is_array($gitInfo) AND array_key_exists($channelid, $gitInfo))
				{
					...
					// Allow user to see their own, unapproved nodes. TODO: test this branch
					$this->doShowApprovedCheck();
		...
		{
			$this->where[] = $this->make_notequals_filter('id', $channelid);

//			$this->what['is_starter2'] = '(id = starter) as is_starter2';
//			$this->where[] = 'is_starter2 = 0';
		}
	...
	protected function process_view_filters($view, $criteria)
	{
		if (empty($view))
		{
			return;
		...
		switch ($view)
		{
			...
			case vB_Api_Search::FILTER_VIEW_ACTIVITY :
				...
				else
				{
					...
					// See db search's process_view_filters() comments.
					// Inherit the ignored user exclusion to lastcontentnode.
				...
				if (!$datecheck)
				{
					...
					$this->where[] = "lastcontent > " . (vB::getRequest()->getTimeNow() - ($age * 86400));
				}
	...
	// set to protected instead of private so that any subclass can implement their own if different behavior is desired.
	protected function adjustKeywordsForSphinx($keywords)
	{
	...
	protected function compute_changes(&$node)
	{
	...
	private function quote_smart($field, $value, $is_handled = true)
	{
		...
		//id is a special field.  We don't to add it ot the attributes list because we assume items therein
		//are actually Sphinx attributes elsewhere and id is not.  It should act like a uint attribute
		//even though it technically isn't
		if($field == 'id')
		{
			$type = 'rt_attr_uint';
		}
		else if (isset($this->attributes_definition[$field]))
		{
			$type = $this->attributes_definition[$field];
		}
		else
		{
			//shouldn't happen but if we're getting unknown fields we could have a problem if we aren't
			//correctly quoting/escaping them
			throw new vB_Exception_Api('invalid_data_w_x_y_z', [$field, 'field', __CLASS__, __FUNCTION__]);
		}

		if ($type == 'rt_attr_string')
		{
			if (empty($value))
			{
				return "''";
	...
	protected function setNodePermTerms($excludeUserSpecific = false)
	{
		...
		{
			$userid = intval($userid);
			// This is OK because this is only in the SELECT, not WHERE.
			// If we need to do the OR check on its own in the WHERE, use doShowApprovedCheck()
	...
	public function sphinxEscapeString($toEscape)
	{
		/*
			...
			There are some more operators in that list, like
				<< (strict order operator)
				: (part of ZONE: and ZONESPAN: limit operators)
			but in some quick testing, these characters either never make it into the search core,
			or do not seem to cause problems when dangling out of context. Leaving the current list
			unchanged, but leaving the reference link here in case we hit more characters in the
			wild that we need to verify & add.
		 */
