...
if ($_REQUEST['do'] == 'sigpic')
{
	...
	if ($userinfo['sigpic'])
	{
		$userinfo['sigpicurl'] = htmlspecialchars('filedata/fetch?filedataid=' . $userinfo['sigpicfiledataid'] . '&sigpic=1');
...
if ($_REQUEST['do'] == 'avatar')
{
	...
	if (!empty($avatarchecked[0]))
	{
		...
		print_label_row($vbphrase['custom_avatar'], '<img src="core/' . $avatar['avatarpath'] . '" alt="" border="0" />');
	}
...
{
	$pms = vB_Api::instance('user')->fetchUsersPms([
		'sortby' => 'total',
		'sortdir' => vB_dB_Query::SORT_DESC,
	]);

	print_stop_message_on_api_error($pms);

	print_form_header('admincp/usertools', 'viewpmstats');
	print_table_header($vbphrase['private_message_statistics_gcpuser'], 3);
	print_cells_row([$vbphrase['number_of_messages'], $vbphrase['number_of_users'], $vbphrase['controls']], 1);

	$groups = array_count_values(array_column($pms, 'total'));

	$pmquota = vB::getDatastore()->getOption('pmquota');
	$keysuffix = ($pmquota ? '/' . $pmquota : '');
	...
	foreach ($groups AS $key => $total)
	{
		...
		$cell[] = $key . $keysuffix;
		$cell[] = $total;
		$cell[] = construct_link_code(construct_phrase($vbphrase['list_users_with_x_messages'], $key), "usertools.php?do=pmuserstats&total=$key");
...
if ($_REQUEST['do'] == 'pmuserstats')
{
	...
	$users = vB_Api::instance('user')->fetchUsersPms([
		'sortby' => 'username',
		'direction' => vB_dB_Query::SORT_ASC,
		'total' => $vbulletin->GPC['total'],
	]);

	print_stop_message_on_api_error($users);
	...
	print_cells_row([$vbphrase['username'], $vbphrase['last_activity'], $vbphrase['options']], 1);
...
if ($_REQUEST['do'] == 'referrers')
{
	...
	print_table_header($vbphrase['referrals_for_time_period']);
	...
	print_form_header('admincp/usertools', 'showreferrals');
	print_table_header($vbphrase['referrals_by_user']);
	print_description_row($vbphrase['please_input_referrer_username']);
	print_input_row($vbphrase['username'], 'referrername');
	print_submit_row($vbphrase['find']);
}
...
if ($_POST['do'] == 'showreferrers')
{
	...
	try
	{
		$users = vB_Api::instanceInternal('referuser')->fetchReferrers($vbulletin->GPC['startdate'], $vbulletin->GPC['enddate']);
	}
	catch (vB_Exception_Api $ex)
	{
		print_stop_message2($ex->getMessage());
	...
	if (!count($users))
	{
		print_stop_message2('no_referrals_matched_your_query', 'usertools', ['do' => 'referrers']);
	}
	else
	{
		print_form_header('admincp/', '');
		print_table_header($vbphrase['referrals_guser'] . ' - ' .	$refperiod, 3);
		print_cells_row([$vbphrase['username'], $vbphrase['referrals_guser'], $vbphrase['total']], 1);
		foreach ($users AS $user)
		{
			$cells = [];
			$cells[] = '<a href="admincp/user.php?do=edit&amp;u=' . $user['userid'] . '">' . $user['username'] . '</a>';
			$cells[] = '<a href="admincp/usertools.php?do=showreferrals&amp;referrerid=' . $user['userid'] . '&amp;startdate=' . $datestartText . '&amp;enddate=' . $dateendText . '">' . $vbphrase['view_referrals'] . '</a>';
			$cells[] = vb_number_format($user['count']);
			print_cells_row($cells);
		}
		print_table_footer(3, '<a class="button" href="admincp/usertools.php?do=referrers">' . $vbphrase['search_again'] . '</a>');
	}
}
...
if ($_REQUEST['do'] == 'showreferrals')
{
	$vbulletin->input->clean_array_gpc('r', [
		...
		'referrername' => vB_Cleaner::TYPE_STR,
	]);
	...
	if (!$referrerid)
	{
		$user = $assertor->getRow('user', ['username' => $vbulletin->GPC['referrername']]);
	...
	try
	{
		$userInfo = vB_Api::instanceInternal('user')->fetchUserinfo($referrerid);
	}
	catch (vB_Exception_Api $ex)
	{
		print_stop_message2($ex->getMessage());
	...
	$conditions = [
		...
		['field' => 'usergroupid', 'value' => [3,4], 'operator' =>  vB_dB_Query::OPERATOR_NE],
	];

	if(!empty($vbulletin->GPC['startdate']))
	{
		$conditions[] = ['field' => 'joindate', 'value' => $vbulletin->GPC['startdate'], 'operator' =>  vB_dB_Query::OPERATOR_GTE];
	}

	if(!empty($vbulletin->GPC['enddate']))
	{
		$conditions[] = ['field' => 'joindate', 'value' => $vbulletin->GPC['enddate'], 'operator' =>  vB_dB_Query::OPERATOR_LTE];
	}

	$users = $assertor->getRows('user',
		[
			vB_dB_Query::COLUMNS_KEY => ['username', 'posts', 'userid', 'joindate', 'lastvisit', 'email'],
			vB_dB_Query::CONDITIONS_KEY => $conditions,
		],
		['field' => 'joindate', 'direction' => vB_dB_Query::SORT_DESC]
	);

	print_form_header('admincp/', '');
	print_table_header($vbphrase['referred_by']);
	$user = $assertor->getRow('user', ['userid' => $referrerid]);
	$ulink = 'admincp/user.php?do=edit&amp;u=' . $user['userid'];
	if ($user['referrerid'])
	{
		$referrer = $assertor->getRow('user', ['userid' => $user['referrerid']]);
		$rlink = 'admincp/user.php?do=edit&amp;u=' . $referrer['userid'];
		print_description_row(construct_phrase($vbphrase['w_x_was_referred_by_y_z'], $user['username'], $ulink, $referrer['username'], $rlink));
	}
	else
	{
		print_description_row(construct_phrase($vbphrase['x_y_was_not_referred'], $user['username'], $ulink));
	}
	print_table_footer();
	...
	if (count($users) > 0)
	{
		foreach ($users AS $user)
		{
			$cells = [];
			$cells[] = '<a href="admincp/user.php?do=edit&amp;u=' . $user['userid'] . '">' . $user['username'] . '</a>';
			$cells[] = vb_number_format($user['posts']);
			$cells[] = '<a href="mailto:' . $user['email'] . '">' . $user['email'] . '</a>';
			$cells[] = '<span class="smallfont">' . vbdate($vbulletin->options['dateformat'] . ', ' . $vbulletin->options['timeformat'], $user['joindate']) . '</span>';
			$cells[] = '<span class="smallfont">' . vbdate($vbulletin->options['dateformat'] . ', ' . $vbulletin->options['timeformat'], $user['lastvisit']) . '</span>';
			print_cells_row($cells);
		}
	}
	else
	{
		echo '<tr><td colspan="5" align="center" class="' . fetch_row_bgclass() . '"><br /><br />' . $vbphrase['no_referrals_found'] . '<br /><br /><br /></td></tr>';
	}

	print_table_footer(5, '<a class="button" href="admincp/usertools.php?do=referrers">' . $vbphrase['search_again'] . '</a>');
}
