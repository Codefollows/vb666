...
if ($_REQUEST['do'] == 'add' OR $_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'editglobal')
{
	...
	else if (empty($vbulletin->GPC['moderatorid']))
	{
		...
		{
			$moderator["$permission"] = ($option['default'] ?? false) ? 1 : 0;
		}
	...
	print_yes_no_row($vbphrase['can_set_answer'], 'modperms[cansetanswer]', $moderator['cansetanswer']);
...
if ($_POST['do'] == 'update')
{
	...
	foreach (array_keys($moddata_dms) AS $dmkey)
	{
		...
		if (!empty($moddata->errors))
		{
			...
			$html .= '<ul><li>' . implode("</li>\n<li>", $moddata->errors) . '</li></ul>';
			$html .= '</ul>';
			$dm_errors .= $html;
		}
		else
	...
	if (!empty($dm_errors))
	{
		...
		{
			print_description_row("$vbphrase[the_following_users_were_made_moderators]<blockquote><ul><li>" . implode("</li>\n<li>", $not_affected) . '</li></ul></blockquote>');
		}
		print_table_footer();
		print_cp_footer();
		exit;
...
if ($_REQUEST['do'] == 'showmods')
{
	...
	if (count($channels))
	{
		...
		{
			$modlist[$channel['userid']] = ($modlist[$channel['userid']] ?? 0) + 1;
