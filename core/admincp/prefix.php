...
define('CVS_REVISION', '$RCSfile$ - $Revision: 112711 $');

// #################### PRE-CACHE TEMPLATES AND DATA ######################
global $phrasegroups, $specialtemplates, $vbphrase, $vbulletin;
$phrasegroups = ['prefix', 'prefixadmin'];
...
if ($_REQUEST['do'] == 'permissions')
{
	...
	print_description_row('<label><input class="js-checkbox-master" type="checkbox"' . (empty($usergroupperms) ? ' checked="checked"' : '') . " />$vbphrase[check_uncheck_all]</label>", false, 2, 'thead');
	...
	foreach ($vbulletin->usergroupcache AS $usergroupid => $usergroup)
	{
		...
		{
			print_label_row(
				"<label><input type=\"checkbox\" disabled=\"disabled\"/>$usergroup[title]</label>",
		...
		{
			print_description_row("<label><input type=\"checkbox\" name=\"usergroup[$usergroupid]\" class=\"js-checkbox-child\"" . (empty($usergroupperms["$usergroupid"]) ? ' checked="checked"' : '') . " />$usergroup[title]</label>");
...
if ($_REQUEST['do'] == 'list')
{
	...
	$prefixsets = [];
	$result = $assertor->select('vBForum:prefixset', [], 'displayorder');
	foreach($result AS $prefixset)
	{
		$prefixsets[$prefixset['prefixsetid']] = $prefixset;
		$prefixsets[$prefixset['prefixsetid']]['prefixes'] = [];
	}

	$result = $assertor->select('vBForum:prefix', [], 'displayorder');
	...
	$colspan = 3;
	print_form_header2('admincp/prefix', 'displayorder');
	print_table_start();
	print_table_header($vbphrase['thread_prefixes_gprefixadmin'], $colspan);
	...
	else
	{
		...
		{
			$setid = $prefixset['prefixsetid'];
			$setname = htmlspecialchars_uni($vbphrase['prefixset_' . $setid . '_title']);

			$cells = [];
			$cells[] = construct_checkbox_control($setname, '', false, 0, ['data-child' => 'js-checkbox-child-' . $setid, 'class' => 'js-checkbox-master']);
			$cells[] = construct_input('text', 'prefixset_order[' . $setid . ']', ['size' => 3, 'value' => $prefixset['displayorder']]);
			$cells[] = '<div class="normal">' .
				construct_link_code2($vbphrase['add_prefix'], get_admincp_url('prefix', ['do' => 'addprefix', 'prefixsetid' => $setid])) .
				construct_link_code2($vbphrase['edit'], get_admincp_url('prefix', ['do' => 'editset', 'prefixsetid' => $setid])) .
				construct_link_code2($vbphrase['delete'], get_admincp_url('prefix', ['do' => 'deleteset', 'prefixsetid' => $setid])) .
				'</div>';
			...
			{
				print_description_row(construct_phrase($vbphrase['no_prefixes_defined_click_create'], $prefixset['prefixsetid']), false, $colspan, '', 'center');
			}
			else
			{
				foreach ($prefixset['prefixes'] AS $prefix)
				{
					...
					$prefixname = htmlspecialchars_uni($vbphrase['prefix_' . $prefixid . '_title_plain']);

					$cells = [];
					$cells[] = construct_checkbox_control($prefixname, 'prefixids[' . $prefixid . ']', false, 0, ['class' => 'js-checkbox-child-' . $setid]);
					$cells[] = construct_input('text', 'prefix_order[' . $prefixid . ']', ['size' => 3, 'value' => $prefix['displayorder']]);
					$cells[] = '<div class="smallfont">' .
						construct_link_code2($vbphrase['edit'], get_admincp_url('prefix', ['do' => 'editprefix', 'prefixid' => $prefixid])) .
						construct_link_code2($vbphrase['delete'], get_admincp_url('prefix', ['do' => 'deleteprefix', 'prefixid' => $prefixid])) .
						construct_link_code2($vbphrase['edit_permissions'], get_admincp_url('prefix', ['do' => 'permissions', 'prefixid' => $prefixid])) .
						'</div>';
	...
	$buttons = [
		construct_submit_button($vbphrase['edit_selected_prefix_permissions'], [], ['name' => 'dopermissionsmultiple']),
		construct_submit_button($vbphrase['save_display_order']),
		construct_link_button($vbphrase['add_prefix_set'] , get_admincp_url('prefix', ['do' => 'addset'])),
	];
