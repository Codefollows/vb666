...
if ($_REQUEST['do'] == 'edit')
{
	...
	print_yes_no_row($vbphrase['can_use_this_extension'], 'useextension', $getperms['attachmentpermissions'], ['onclick' => 'js_set_custom();']);
...
if ($_POST['do'] == 'doupdate')
{
	...
	if ($vbulletin->GPC['useusergroup'])
	{
		...
		if ($vbulletin->GPC['attachmentpermissionid'])
		{
			vB::getDbAssertor()->delete('vBForum:attachmentpermission', array('attachmentpermissionid' => $vbulletin->GPC['attachmentpermissionid']));
...
if ($_REQUEST['do'] == 'modify')
{
	...
	foreach ($attachments as $attach)
	{
		...
		echo "$indent<li><b><a name=\"a_$extension\" href=\"admincp/attachment.php?do=updatetype&amp;extension=$extension\">$extension</a></b>";
		echo "<span class=\"smallfont\">(" .
			construct_link_code($vbphrase['reset'], "attachmentpermission.php?do=quickset&amp;type=reset&amp;extension=$extension&amp;hash=" . CP_SESSIONHASH) .
			construct_link_code($vbphrase['deny_all'], "attachmentpermission.php?do=quickset&amp;type=deny&amp;extension=$extension&amp;hash=" . CP_SESSIONHASH) .
			")</span></b>";
		...
		foreach ($vbulletin->usergroupcache AS $usergroupid => $usergroup)
		{
			...
			echo "$indent\t<li$class>" . construct_link_code($vbphrase['edit'], "attachmentpermission.php?do=edit&amp;$link") . $usergroup['title'] . "</li>\n";
...
if ($_REQUEST['do'] == 'quickset')
{
	...
	$attachment_type = $assertor->getRow('vBForum:attachmenttype', ['extension' => $vbulletin->GPC['extension']]);
	...
	switch ($vbulletin->GPC['type'])
	{
		...
		case 'deny':
			$groupids = $assertor->getColumn('usergroup', 'usergroupid', []);
			$assertor->assertQuery('vBForum:replacePerms', [
				'extension' => $vbulletin->GPC['extension'],
				'size' => $attachment_type['size'],
				'width' => $attachment_type['width'],
				'height' => $attachment_type['height'],
				'groupids' => $groupids,
				'attachmentpermissions' => 0,
			]);
			break;
	...
	print_stop_message2('saved_attachment_permissions_successfully', 'attachmentpermission', ['do'=>'modify#a_' . $vbulletin->GPC['extension']]);
}
