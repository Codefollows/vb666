...
define('CVS_REVISION', '$RCSfile$ - $Revision: 115977 $');
...
// ############################# LOG ACTION ###############################
log_admin_action($vbulletin->GPC['dostyleid'] != 0 ? 'style id = ' . $vbulletin->GPC['dostyleid'] : '');
...
{
	$styleinfo = $assertor->getRow('vBForum:style', ['styleid' => $vbulletin->GPC['dostyleid']]);
	if (empty($styleinfo))
	{
...
print_cp_header($vbphrase['stylevareditor'], '', [
	...
	'<script type="text/javascript" src="core/clientscript/vbulletin_stylevars.js?v=' . $vboptions['simpleversion'] .'"></script>'
]);


function construct_stylevar_form($title, $stylevarid, $values, $styleid)
{
	$stylecache = vB_Library::instance('Style')->fetchStyles(false, false, ['skipReadCheck' => true]);
	...
	else
	{
		while (!isset($values[$stylevarid][$styleid]))
		{
			...
			{
				//an untyped Exceptions is better than a PHP error via trigger_error
				throw new Exception('Invalid style in tree: ' . $styleid);
			}
		}
		$hide_revert = true;
	}
	...
	else if ($stylevar['stylevarstyleid'] == $styleid)
	{
...
if ($_REQUEST['do'] == 'dfnadd' OR $_REQUEST['do'] == 'dfnedit')
{
	...
	if ($vbulletin->GPC['stylevarid'])
	{
		...
		$stylevar = $assertor->getRow('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['stylevarid'], 'styleid' => $vbulletin->GPC['dostyleid']]);

		if (!empty($stylevar))
		{
			// select friendly name for current language
			$svname_result = $assertor->getRow('vBForum:phrase', [
			...
			$svdesc_result = $assertor->getRow('vBForum:phrase', [
	...
	print_textarea_row($vbphrase['description_gcpglobal'], 'svdescription', $stylevar['description']);
	// keys match with enum entry that we have, value should be mapped to a vbphrase
	$svtypesarray = [
		$vbphrase['simple_types'] => [
...
if ($_POST['do'] == 'dfn_dosave')
{
	...
	{
		$stylevar_dfn = $assertor->getRow('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['stylevarid']]);

		if (!empty($stylevar_dfn))
		{
			print_stop_message2(['stylevar_x_already_exists', $vbulletin->GPC['stylevarid']]);
		}
	}
	...
	// time to store it
	$svdfndata = new vB_Datamanager_StyleVarDefn(vB_DataManager_Constants::ERRTYPE_CP);
	if ($vbulletin->GPC['oldsvid'])
	{
		//save this for later because it's gone after we update
		$stylevarinfo = $assertor->getRow('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['oldsvid']]);

		$existing = ['stylevarid' => $vbulletin->GPC['oldsvid']];
		$svdfndata->set_existing($existing);

		if (defined('DEV_AUTOEXPORT') AND DEV_AUTOEXPORT)
		{
			$stylevar_dfn = $assertor->getRow('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['oldsvid']]);
		}
	}
	...
	//we'll rebulid the language below so skip it here.
	$phraseLib = vB_Library::instance('phrase');
	$phraseLib->saveMaster($vbulletin->GPC['product'], 'style', 'stylevar_' . $dfnid . '_name', $vbulletin->GPC['svfriendlyname'], true);
	$phraseLib->saveMaster($vbulletin->GPC['product'], 'style', 'stylevar_' . $dfnid . '_description', $vbulletin->GPC['svdescription'], true);
	...
	if (!$vbulletin->GPC['oldsvid'])
	{
		...
		$stylevardata = new vB_Datamanager_Stylevar(vB_DataManager_Constants::ERRTYPE_CP);
	...
	$autoexportProducts = [$vbulletin->GPC['product']];
	if (isset($stylevar_dfn['product']))
	{
		$autoexportProducts[] = $stylevar_dfn['product'];
	}
	...
	{
		// copy the old stylevar value to the new stylevar and delete the old one.
		// (not sure why we don't just update the id)
		$oldValue = $assertor->getRow('vBForum:stylevar', [
		...
		//don't need to delete the stylevardfn here -- the datamanager save changed the id so oldsvid
		//no longer exists there.
		$assertor->delete('vBForum:stylevar', ['stylevarid' => $vbulletin->GPC['oldsvid']]);
		...
		$assertor->assertQuery('vBForum:phrase', [
			...
			'fieldname' => 'style',
			'varname' => [
				'stylevar_' . $vbulletin->GPC['oldsvid'] . '_name',
				'stylevar_' . $vbulletin->GPC['oldsvid'] . '_description',
			],
		]);
	}
	...
	print_stop_message2(['saved_stylevardfn_x_successfully', $vbulletin->GPC['stylevarid']], 'stylevar',
		['do' => 'fetchstylevareditor', 'dostyleid' => $vbulletin->GPC['dostyleid'], 'stylevarid[]' => $vbulletin->GPC['stylevarid']]);
}
...
if ($_REQUEST['do'] == 'confirmrevert')
{
	...
	print_delete_confirmation(
		'stylevar',
		$vbulletin->GPC['stylevarid'],
		'stylevar',
		'dorevert',
		'stylevar',
		$hidden,
		$vbphrase['please_be_aware_stylevar_is_inherited']
	);
}
...
if ($_POST['do'] == 'dorevert')
{
	...
	if ($vbulletin->GPC['dostyleid'] == -1)
	{
		...
		$stylevarinfo = $assertor->getRow('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['stylevarid']]);

		$assertor->delete('vBForum:stylevar', ['stylevarid' => $vbulletin->GPC['stylevarid']]);
		$assertor->delete('vBForum:stylevardfn', ['stylevarid' => $vbulletin->GPC['stylevarid']]);
		...
		$assertor->assertQuery('vBForum:phrase', [
			...
			'fieldname' => 'style',
			'varname' => [
				'stylevar_' . $vbulletin->GPC['stylevarid'] . '_name',
				'stylevar_' . $vbulletin->GPC['stylevarid'] . '_description',
			],
		]);
		...
		print_stop_message2(['reverted_stylevar_x_successfully', $vbulletin->GPC['stylevarid']], 'stylevar', ['dostyleid' => $vbulletin->GPC['dostyleid']]);
	}
	else
	{
		$assertor->delete('vBForum:stylevar', ['stylevarid' => $vbulletin->GPC['stylevarid'], 'styleid' => intval($vbulletin->GPC['dostyleid'])]);

		print_rebuild_style($vbulletin->GPC['dostyleid']);
		print_stop_message2(['reverted_stylevar_x_successfully', $vbulletin->GPC['stylevarid']], 'stylevar',
			['dostyleid' => $vbulletin->GPC['dostyleid'], 'do' => 'fetchstylevareditor',	'stylevarid[]' => $vbulletin->GPC['stylevarid']]);
	}
}
...
if ($_POST['do'] == 'savestylevar')
{
	...
	$stylevars_result = $assertor->assertQuery('vBForum:getExistingStylevars', ['stylevarids' => $stylevarids]);
	...
	{
		$existing_result = $assertor->getRows('vBForum:stylevar', [
		...
		$existing_dfns = $assertor->getRows('vBForum:stylevardfn', [
		...
		foreach ($updated_stylevars AS $stylevarid)
		{
			...
			{
				$svexisting = ['stylevarid' => $stylevarid, 'styleid' => $vbulletin->GPC['dostyleid']];
			...
			foreach ($update_properties AS $property)
			{
				...
				)
				{
					...
					//not all params have the inherit param.  this is used for the color transform logic
					$hasInheritedColorParamChanged = (isset($stylevar_vals[$inherit_param_prop_name]) AND ($stylevar_vals[$inherit_param_prop_name] != $stylevar_vals[$orig_inherit_param_prop_name]));
				...
				else if ($stylevar_vals[$property] != $stylevar_vals[$orig_prop_name])
				{
					...
					{
						$errors[] = ['invalid_stylevar_value', $stylevarid, $property];
					}

					// clear inherited value
					$svinstance->set_child($stylevar_prop_name, '');
				}
				//we'll only have the style var prop value in debug mode.  So check if it exists.
				else if (isset($stylevar_vals[$stylevar_prop_name])
					AND $stylevar_vals[$stylevar_prop_name] !== ($original_values[$stylevarid][$stylevar_prop_name] ?? null )
				)
				{
				...
				if (substr($property, -5) == 'color')
				{
					...
					//this is only set in debug mode
					else if (isset($stylevar_vals[$inherit_param_property_name]) AND $stylevar_vals[$inherit_param_property_name] != $stylevar_vals['original_' . $inherit_param_property_name])
					{
						// value was modified
...
if ($_REQUEST['do'] == 'fetchstylevareditor')
{
	...
	$stylevars_result = $assertor->getRows('vBForum:getExistingStylevars', ['stylevarids' => $stylevarids]);

	// Some checks to see if we need to show the download link.
	$groupHasCustomStyleVars = [];
	$groupHasCustomizationInThisStyleid = [];
	$styleParents = $styleinfo['parentlist'];
	$styleParentsByStyleid = [];
...
if ($_REQUEST['do'] == 'showstylevartemplateusage')
{
	...
	foreach ($stylevarGroups AS $group => $stylevars)
	{
		...
		{
			$templates = $assertor->getRows('template', [
				vB_dB_Query::COLUMNS_KEY => ['title'],
				vB_dB_Query::CONDITIONS_KEY => [
					['field' => 'template_un', 'value' => $stylevarid, 'operator' => vB_dB_Query::OPERATOR_INCLUDES],
					'styleid' => -1,
				],
			], 'title');
			$templateNames = [];
...
if ($_POST['do'] == 'dorevertall')
{
	...
	if ($vbulletin->GPC['dostyleid'] != -1 AND $style = $assertor->getRow('vBForum:style', ['styleid' => $vbulletin->GPC['dostyleid']]))
	{
		if (!$style['parentlist'])
		{
			$style['parentlist'] = '-1';
		...
		$stylevars = $assertor->getRows('vBForum:getStylevarsToRevert', [
		...
		else
		{
			...
			{
				vB::getDbAssertor()->assertQuery('vBForum:stylevar', [
					vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_DELETE,
					'stylevarid' => $deletestylevars,
					'styleid' => $style['styleid']
				]);
