...
if ($_REQUEST['do'] == 'choose')
{
	...
	{
		$title = construct_phrase($vbphrase['rename_table_x'], $table[0]);
		$name = "rename[$table[0]]";
		$prefix = substr($table[0], 0, $prefixlength);
		$value = ($prefix == TABLE_PREFIX ? 1 : 0);
...
function fetch_renamed_table_name($table, $oldprefix, $newprefix, $dobold = false)
{
	...
	$name['new'] = ($dobold ? "<b>$newprefix</b>" : $newprefix) . $table;
