...
class vB_SignatureParser_CharCount extends vB_SignatureParser
{
	...
	*/
	function parse_bbcode($input_text, $do_smilies, $do_imgcode, $do_html = false, $do_censor = true)
	{
		...
		return parent::parse_bbcode($input_text, $do_smilies, $do_imgcode, $do_html, $do_censor);
	}
	...
	*/
	...
	/*
		contains_bbcode_img_tags() does not currently count img2 as "imgs"
		(img2 tags are handled via handle_bbcode_img2 per
		vB_Api_Bbcode::fetchTagList() return), so we can't stick this
		in handle_bbcode_img() without impacting the other parsers too.

		Note, right now the sigmaxrawchars checks are NOT performed with
		this parser, only the sigmaxlines checks. Captioned images are
		usually multiple lines "tall" and has whitespace around it.
		Since the image itself doesn't seem to contribute towards the
		linecount, I guess the caption should only count as one line?
		Depending on the image size the caption might be multiple lines,
		but that currently does NOT work correctly (caption does not wrap)

		Edit: nevermind. This class's append_custom_tags() overwrites ALL
		bbcodes, not just the custom bbcodes, with
			callback => handle_standard_tag
		so none of the actual handle_bbcode_<...> functions are getting hit.
		This is too much to unwrap right now, so I'm just leaving this here
		and stopping.
	 */
	/*
	protected function getImageHtml($settings, $link, $size)
	{
		$caption = $settings['all']['caption'] ?? '';
		if (!empty($caption))
		{
			// Ensure caption is on its own line
			$caption = "\n$caption\n";
		}
		return $caption;
	}
