...
// We really should get rid of these global reference variables. That screws things up on the otherside of the reference
// if a script downstream re-uses $db in global context.
$db = &$vbulletin->db;
...
{
	exec_header_redirect(vB5_Route::buildHomeUrl('fullurl'), 302);
}
...
// I don't think the MAPI code can reach this any more.  We handle things differently now
// Because of Signature Verification, VB API won't need to verify securitytoken
// CSRF Protection for POST requests
if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST' AND !(defined('VB_API') AND VB_API))
{
