...
{
	return vB_Library::instance('language')->buildLanguageCache();
}
...
function fetch_custom_phrases($languageid, $fieldname = '')
{
	...
	return fetch_phrases_internal($languageid, 0, $fieldname, 0);
}
...
{
	return fetch_phrases_internal($languageid, -1, $fieldname, $offset);
}

function fetch_phrases_internal($languageid, $type, $fieldname, $offset)
{
	$phrases = vB::getDbAssertor()->assertQuery('fetchPhrases', [
		'languageid' => $languageid,
		'fieldname' => $fieldname,
		'type' => $type,
	]);
...
function get_language_export_xml($languageid, $product, $custom, $just_phrases, $charset = 'UTF-8')
{
	...
	{
		$language = [
			'title' => new vB_Phrase('language', 'master_language'),
			'vblangcode' => '',
			'revision' => '',
		];
	...
	{
		$language = $db->getRow('language', ['languageid' => $languageid]);
	}

	$title = str_replace('"', '\"', $language['title']);
	$version = str_replace('"', '\"', $datastore->getOption('templateversion'));
	...
	$getphrases = $db->assertQuery('fetchPhrasesForExport', [
		'languageid'             => $languageid,
		'custom'                 => $custom,
		'product'                => $product,
...
*/
function xml_import_language($xml, $languageid = -1, $title = '', $anyversion = false, $userselect = true, $output = true)
{
	//need to pass this in as a parameter or rewrite things so it doesn't matter.
	//in some cases we want to output the keepalive spaces but not any other output
	$showmessages = !(defined('VB_AREA') AND (VB_AREA == 'Install' OR VB_AREA == 'Upgrade'));

	global $vbulletin;

	$vbphrase = vB_Api::instanceInternal('phrase')->renderPhrasesNoShortcode([
		'please_wait' => 'please_wait',
		'importing_language' => 'importing_language',
	...
	if ($major_version <= 4)
	{
		// Seems like this import function does not have a "silent"/exception mode. It might be used in loop
		// in the upgrader (e.g. translation imports) where we don't want the print..() but there doesn't seem
		// to be a flag we can check for that... so we'll just print_stop_message() here like the other errors
		// in this function.
		print_dots_stop();
	...
	$langinfo['options'] = intval($langinfo['options'] ?? 0);
	...
	else
	{
		if ($languageid == 0)
		{
			...
			if ($output AND $showmessages)
			{
				$phrase = vB_Api::instanceInternal('phrase')->renderPhrasesNoShortcode([
					['creating_a_new_language_called_x', $title]
				]);
				$phrase = reset($phrase['phrases']);
				echo "<h3><b>" . $phrase . "</b></h3>\n<p>$vbphrase[please_wait]</p>";
				vbflush();
			}
		...
		else
		{
			...
			if ($getlanguage = $vbulletin->db->query_first("SELECT title FROM " . TABLE_PREFIX . "language WHERE languageid = $languageid"))
			{
				if (!$just_phrases)
				{
					...
					{
						$phrase = vB_Api::instanceInternal('phrase')->renderPhrasesNoShortcode([
							['overwriting_language_x', $getlanguage['title']]
						]);
						$phrase = reset($phrase['phrases']);
						echo "<h3><b>" . $phrase . "</b></h3>\n<p>$vbphrase[please_wait]</p>";
						vbflush();
	...
	$current_phrasetypes = vB_Api::instanceInternal('phrase')->fetch_phrasetypes();
	...
	if (!$master)
	{
		...
		{
			$globalPhrases[$getphrase['varname']] = true;
		}
	}
	...
	foreach (array_keys($arr) AS $key)
	{
		...
		{
			$phrase['date'] = $phrase['date'] ?? 0;
			$phrase['username'] = $phrase['username'] ?? '';
			$phrase['version'] = $phrase['version'] ?? '';

			if ($master)
			{
				$insertLanguageId = -1;
			}
			else if (!isset($globalPhrases[$phrase['name']]))
			{
				$insertLanguageId = 0;
			}
			else if (!empty($phrase['custom']))
			{
...
*/
function print_phrase_row($phrase, $editrows, $key, $dir)
{
	global $vbphrase;
	static $bgcount = 0;
