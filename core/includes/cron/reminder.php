...
$events = vB::getDbAssertor()->assertQuery('fetchEvents', ['beginday' => $beginday, 'endday' => $endday]);
...
if (!empty($updateids))
{
	vB::getDbAssertor()->assertQuery('updateSubscribeEvent' , ['conditions' => $sql, 'subscribeeventid' => $updateids]);
}
...
foreach ($eventlist AS $userid => $event)
{
	...
	// Updating this phrase for displaynames, but vB5 events do not seem to be hooked up to reminders/subscribeevent yet, so no way to test this.
	// In fact this file doesn't seem to be registered with any default vB5 crons.
	$maildata = $phraseAPI->fetchEmailPhrases(
		'reminder',
		[
			$displayname_safe,
	...
	// TODO: if we restore this cron, check mailoption opt-out
